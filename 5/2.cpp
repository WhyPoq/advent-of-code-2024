        ...
        else
        {
            ...
            for (int i = 0; i < print_order.size(); i++)
            {
                ...
                if (page_requirements.find(page_to_print) != page_requirements.end())
                {
                    ...
                    {
                        // if needed page in print order and we have not yet printed it
            ...
            if (!good)
            {
                ...
                while (right_order.size() < print_order.size())
                {
                    for (int i = 0; i < print_order.size(); i++)
                    {
                        int page_to_print = print_order[i];
                        // if the page has not yet been included in the right order
                        if (already_printed.find(page_to_print) == already_printed.end())
                        {
                            bool can_print = true;
                            if (page_requirements.find(page_to_print) != page_requirements.end())
                            {
                            ...
                            {
                                already_printed.insert(page_to_print);
                                right_order.push_back(page_to_print);
                            }
                        }
                    }
                }

                ans += right_order[right_order.size() / 2];
            }
        }
    }
